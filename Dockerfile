# ─────────────────────────────────────────────────────────────────────────────
# ClinicalVault FHIR Server – production image
#
...
#
# TARGET PLATFORM: linux/amd64 (ECS Fargate cpu_architecture=X86_64)
# Always build with:
#   docker buildx build --platform linux/amd64 ...
# CI/CD enforces this via: platforms: linux/amd64 in ci-build.yml
#
...
RUN apt-get update -qq \
    && apt-get install -y --no-install-recommends curl gettext-base python3 \
...
RUN rm -f  ${SERVER_DIR}/apps/fhir-server.war.xml \
    && rm -f ${SERVER_DIR}/dropins/fhir-server.war.xml \
    ...
    && rm -f ${SERVER_DIR}/configDropins/overrides/jwtRS.xml \
...
# Copy TLS key/trust stores referenced by server.xml.
COPY fhir-server-webapp/src/main/liberty/config/resources/security/fhirKeyStore.p12 \
    ${SERVER_DIR}/resources/security/fhirKeyStore.p12
COPY fhir-server-webapp/src/main/liberty/config/resources/security/fhirTrustStore.p12 \
    ${SERVER_DIR}/resources/security/fhirTrustStore.p12
...
# ── 3b. Install Liberty features declared in server.xml ──────────────────────
#    The Maven liberty:install-feature goal only assembled a minimal EE 6/7
#    base runtime.  Run featureUtility to download and install all EE 8 /
#    MicroProfile features (servlet-4.0, jaxrs-2.1, appSecurity-2.0,
#    mpJwt-1.2, etc.) from the Open Liberty Maven repository at build time.
#    This runs as root (before USER 1001) so it can write to ${WLP_DIR}.
RUN ${WLP_DIR}/bin/featureUtility installServerFeatures \
        --acceptLicense \
        defaultServer

# ── 3c. Bake in the schema CLI jar ────────────────────────────────────────────
