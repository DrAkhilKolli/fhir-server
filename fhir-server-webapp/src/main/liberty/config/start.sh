...
# to remove stale featureManager blocks and enforce authFilterRef + failover settings.
...
if os.path.exists(path):
    ...
    # Stale object-store dropins may still contain a featureManager section with
    # mpJwt-2.1. Keep feature declarations exclusively in server.xml to avoid
    # Liberty singleton conflicts (transaction-1.2 vs transaction-2.0, etc.).
    content = re.sub(r'<featureManager>.*?</featureManager>\s*', '', content, flags=re.DOTALL | re.IGNORECASE)
    content = re.sub(r'<feature>\s*mpJwt-[^<]*</feature>\s*', '', content, flags=re.IGNORECASE)
